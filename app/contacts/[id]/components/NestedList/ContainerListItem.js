...
import {ListItemIcon,Checkbox,ListItemButton,Collapse,ListItemText} from '@mui/material';
...
const ContainerListItem = ({ item,level=0 }) => {
    ...
    const subitems = data.filter((x) => x.id === item.id && x.children && x.children.length > 0);
    ...
    return (
      <ListItemButton sx={{ pl: level*4 }}  onClick={()=>onSelect(item.id)}>
        <ListItemIcon>
          <Checkbox
            edge="start"
            checked={checked}
